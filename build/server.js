...
var UserSchema = import_zod3.z.object({
  name: import_zod3.z.string({ required_error: "O nome \xE9 obrigat\xF3rio." }),
  email: import_zod3.z.string({ required_error: "O e-mail \xE9 obrigat\xF3rio." }).email({ message: "O e-mail \xE9 inv\xE1lido." }),
  password: import_zod3.z.string({ required_error: "A senha \xE9 obrigat\xF3ria." }).min(8, { message: "A senha deve ter no m\xEDnimo 8 caracteres." }).regex(/[a-z]/i, { message: "A senha deve ter pelo menos uma letra." }).regex(/[0-9]/, { message: "A senha deve ter pelo menos um n\xFAmero." }),
  image: import_zod3.z.string({ required_error: "A imagem \xE9 obrigat\xF3ria." }),
  acceptedTerms: import_zod3.z.boolean({ required_error: "Os termos s\xE3o obrigat\xF3rios." }),
...
var UserEmailVerificationSchema = import_zod3.z.object({
  email: import_zod3.z.string({ required_error: "O e-mail \xE9 obrigat\xF3rio." }).email({ message: "O e-mail \xE9 inv\xE1lido." }),
  token: import_zod3.z.string({ required_error: "O token \xE9 obrigat\xF3rio." }).min(6, { message: "O token deve ter 6 caracteres." }).max(6, { message: "O token deve ter 6 caracteres." })
});

// src/DTOs/Post.ts
var import_zod4 = require("zod");
var PostSchema = import_zod4.z.object({
  title: import_zod4.z.string({ required_error: "O t\xEDtulo \xE9 obrigat\xF3rio." }),
  content: import_zod4.z.string({ required_error: "O conte\xFAdo \xE9 obrigat\xF3rio." }),
  authorId: import_zod4.z.string({ required_error: "O id do autor \xE9 obrigat\xF3rio." }),
  published: import_zod4.z.boolean({
...
var PostGetSchema = import_zod4.z.object({
  search: import_zod4.z.string({ invalid_type_error: "O t\xEDtulo deve deve ser um texto." }).optional(),
  page: import_zod4.z.coerce.number({ invalid_type_error: "A p\xE1gina deve deve ser um n\xFAmero." }).int({ message: "A p\xE1gina deve ser um n\xFAmero inteiro." }).min(1, { message: "A p\xE1gina deve ser maior que 0." }).default(1),
  limit: import_zod4.z.coerce.number({ invalid_type_error: "O limite deve deve ser um n\xFAmero." }).int({ message: "O limite deve ser um n\xFAmero inteiro." }).min(1, { message: "O limite deve ser maior que 0." }).default(50),
  isDraft: import_zod4.z.coerce.boolean({
    invalid_type_error: "O status de publica\xE7\xE3o deve ser um booleano."
  }).optional(),
  orderField: import_zod4.z.enum(
    [
      "id",
      "createdAt",
      "updatedAt",
...
var CommentSchema = import_zod5.z.object({
  content: import_zod5.z.string({ required_error: "O conte\xFAdo \xE9 obrigat\xF3rio." }),
  authorId: import_zod5.z.string({ required_error: "O id do autor \xE9 obrigat\xF3rio." }),
  postId: import_zod5.z.string({ required_error: "O id do post \xE9 obrigat\xF3rio." }),
  commentParentId: import_zod5.z.string({
...
var CommentGetSchema = import_zod5.z.object({
  search: import_zod5.z.string({ invalid_type_error: "O t\xEDtulo deve deve ser um texto." }).optional(),
  page: import_zod5.z.coerce.number({ invalid_type_error: "A p\xE1gina deve deve ser um n\xFAmero." }).int({ message: "A p\xE1gina deve ser um n\xFAmero inteiro." }).min(1, { message: "A p\xE1gina deve ser maior que 0." }).default(1),
  limit: import_zod5.z.coerce.number({ invalid_type_error: "O limite deve deve ser um n\xFAmero." }).int({ message: "O limite deve ser um n\xFAmero inteiro." }).min(1, { message: "O limite deve ser maior que 0." }).default(50),
  isEdited: import_zod5.z.coerce.boolean({
    invalid_type_error: "O status de edi\xE7\xE3o deve ser um booleano."
  }).optional(),
  orderField: import_zod5.z.enum(
    [
      "content",
      "authorId",
      "postId",
  ...
  order: import_zod5.z.enum(["asc", "desc"], {
    invalid_type_error: "A ordena\xE7\xE3o deve ser 'asc' ou 'desc'."
  }).optional()
});
...
var UserController = class {
  ...
  async update(req, res, next) {
    try {
      ...
      if (data.password) {
        ...
        if (!isSamePassword) {
          throw (0, import_http_errors5.default)(400, "A senha atual est\xE1 incorreta.");
        }
      }
      if (data.newPassword === data.password) {
        throw (0, import_http_errors5.default)(400, "A senha nova deve ser diferente da atual.");
      }
  ...
  async verifyEmail(req, res, next) {
    try {
      ...
      if (user.emailVerified) {
        throw (0, import_http_errors5.default)(400, "O e-mail j\xE1 foi verificado.");
      }
      const isTokenValid = user.emailVerifyToken === token;
      if (!isTokenValid) {
        throw (0, import_http_errors5.default)(400, "O token \xE9 inv\xE1lido.");
      }
      ...
      if (isTokenExpired) {
        throw (0, import_http_errors5.default)(400, "O token expirou.");
      }
...
var AuthController = class {
  async login(req, res, next) {
    try {
      ...
      const user = await userRepository_default.findByEmail(email);
      if (!user) {
        throw (0, import_http_errors6.default)(400, "Login e/ou senha incorretos.");
      }
      const isPasswordValid = await (0, import_bcryptjs2.compare)(password, user.password);
      if (!isPasswordValid) {
        throw (0, import_http_errors6.default)(400, "Login e/ou senha incorretos.");
      }
      const accessToken = import_jsonwebtoken5.default.sign(
        {
          id: user.id,
          role: user.role
        },
...
var PostController = class {
  ...
  async read(req, res, next) {
    try {
      ...
      if (!post) {
        throw (0, import_http_errors7.default)(404, "Post n\xE3o encontrado.");
      }
      res.status(200).json({
        data: post,
...
var MailController = class {
  async emailVerification(req, res, next) {
    try {
      ...
      const user = await userRepository_default.findByEmail(email);
      if (!user) {
        throw (0, import_http_errors8.default)(404, "Usu\xE1rio n\xE3o encontrado.");
      }
      ...
      const mailResponse2 = await mailHandler_default(options);
      if (!mailResponse2) {
        throw (0, import_http_errors8.default)(503, "Erro ao enviar e-mail.");
      }
      res.status(200).json({
...
var CommentController = class {
  ...
  async readByPost(req, res, next) {
    try {
      ...
      const postExists = await postRepository_default.findById(id);
      if (!postExists) {
        throw (0, import_http_errors9.default)(404, "Post n\xE3o encontrado.");
      }
      res.status(200).json({
        data: {
          comments,
  ...
  async readByUser(req, res, next) {
    try {
      ...
      const userExists = await userRepository_default.findById(id);
      if (!userExists) {
        throw (0, import_http_errors9.default)(404, "Usu\xE1rio n\xE3o encontrado.");
      }
  ...
  async read(req, res, next) {
    try {
      ...
      if (!comment) {
        throw (0, import_http_errors9.default)(404, "Coment\xE1rio n\xE3o encontrado.");
      }
      res.status(200).json({
        data: comment,
...
UserRouter.route("/").post(UserController_default.create).get([auth], UserController_default.readAll);
UserRouter.route("/verify-email").post([auth], UserController_default.verifyEmail);
UserRouter.route("/:id").patch([auth], UserController_default.update).get([auth], UserController_default.read).delete([auth], UserController_default.delete);
var UserRoutes_default = UserRouter;

// src/routes/AuthRoutes.ts
var import_express2 = require("express");
var AuthRouter = (0, import_express2.Router)();
AuthRouter.route("/").post(AuthController_default.login).delete(AuthController_default.logout);
var AuthRoutes_default = AuthRouter;

// src/routes/PostRoutes.ts
var import_express3 = require("express");
var PostRouter = (0, import_express3.Router)();
PostRouter.route("/").post([auth, emailVerify, adminVerify], PostController_default.create).get([auth], PostController_default.readAll);
PostRouter.route("/:id").get([auth], PostController_default.read).patch([auth, emailVerify, adminVerify], PostController_default.update).delete([auth, emailVerify, adminVerify], PostController_default.delete);
PostRouter.route("/user/:id").get([auth], PostController_default.readByUser);
var PostRoutes_default = PostRouter;

// src/routes/MailRoutes.ts
var import_express4 = require("express");
var MailRouter = (0, import_express4.Router)();
...
// src/routes/CommetRoutes.ts
var import_express5 = require("express");
var CommentRouter = (0, import_express5.Router)();
CommentRouter.route("/").post([auth, emailVerify], CommentController_default.create).get([auth, emailVerify], CommentController_default.readAll);
CommentRouter.route("/:id").get([auth, emailVerify], CommentController_default.read).patch([auth, emailVerify], CommentController_default.update).delete([auth, emailVerify], CommentController_default.delete);
CommentRouter.route("/user/:id").get(
  [auth, emailVerify],
  CommentController_default.readByUser
);
CommentRouter.route("/post/:id").get(
  [auth, emailVerify],
  CommentController_default.readByPost
);
var CommetRoutes_default = CommentRouter;

// src/routes/index.ts
var router = (0, import_express6.Router)();
router.use("/user", UserRoutes_default);
router.use("/session", AuthRoutes_default);
router.use("/post", PostRoutes_default);
router.use("/mail", MailRoutes_default);
router.use("/comment", CommetRoutes_default);
router.get("/", (req, res) => {
  res.send("Made by damattag => https://github.com/damattag \u{1F601}\u270C\uFE0F");
});
...
app.use("/docs", import_swagger_ui_express.default.serve, import_swagger_ui_express.default.setup(docs_default));
...
app_default.listen(
  ...
  () => {
    console.log(
      `\u{1F680} Server ready at http://localhost:${process.env.SERVER_PORT}`
    );
  }
);
