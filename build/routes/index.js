...
async function emailVerify(req, res, next) {
  try {
    const authToken = req.headers.authorization;
    if (!authToken) {
      throw (0, import_http_errors8.default)(401, "Token n\xE3o encontrado.");
    }
    const decoded = import_jsonwebtoken4.default.verify(authToken, process.env.JWT_SECRET);
    if (typeof decoded === "string") {
      throw (0, import_http_errors8.default)(401, "Token inv\xE1lido.");
    }
    const { id } = decoded;
    const user = await userRepository_default.findById(id);
    if (!user) {
      throw (0, import_http_errors8.default)(401, "Sem autoriza\xE7\xE3o.");
    }
    if (!user.emailVerified) {
      throw (0, import_http_errors8.default)(
        403,
...
async function adminVerify(req, res, next) {
  try {
    const authToken = req.headers.authorization;
    if (!authToken) {
      throw (0, import_http_errors9.default)(401, "Token n\xE3o encontrado.");
    }
    const decoded = import_jsonwebtoken5.default.verify(authToken, process.env.JWT_SECRET);
    if (typeof decoded === "string") {
      throw (0, import_http_errors9.default)(401, "Token inv\xE1lido.");
    }
    ...
    if (role !== "ADMIN") {
      throw (0, import_http_errors9.default)(
        403,
